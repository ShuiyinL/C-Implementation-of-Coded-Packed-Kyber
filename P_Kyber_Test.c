...
///////////////////////////////////////////////////// Kyber KEM with MMSE Quantization Start ////////////////////////////////////////////////////////////////////////
...
int main() {
	...
	*DWT_CYCCNT = 0; // reset the counter
    ...
    printf("-------------------------------------------------------------------- \n");
    if (KYBER_K == 2) {
        printf("Test P-Kyber using the parameters of KYBER512 \n");
    }
    else if (KYBER_K == 3) {
        printf("Test P-Kyber using the parameters of KYBER768 \n");

    }
    else {
        printf("Test P-Kyber using the parameters of KYBER1024\n");
    }
    ...
    printf("-------------------------------------------------------------------- \n");
	   ...
	   uint8_t key_b[CRYPTO_BYTES];
	    ...
	    printf("-------------------------------------------------------------------- \n");
	    printf("Test 1: Run the original Kyber KEM 8 times to exchange 8 AES Keys \n");
	...
	*DWT_CONTROL |= 0; // close the counter
	numCycle1 = t2 - t1;
	printf("clock cycles for 8 calls of KeyGen: %u \n",
	...
	*DWT_CONTROL |= 0; // close the counter
	numCycle1 = t2 - t1;
	printf("clock cycles for 8 calls of Enc: %u \n",
					numCycle1*8);
	...
	*DWT_CONTROL |= 0; // close the counter
	numCycle1 = t2 - t1;
	printf("clock cycles for 8 calls of Dec: %u \n",
					numCycle1*8);
...
*/
    ...
    ////////////////////////////////////////////////////////////////////
	    ...
	    uint8_t pk_LDKEM[CRYPTO_PUBLICKEYBYTES_LD];
	    uint8_t sk_LDKEM[CRYPTO_SECRETKEYBYTES_LD];
	    uint8_t ct_LDKEM[KYBER_CIPHERTEXTBYTES_LD];
	    uint8_t key_a_LDKEM[CRYPTO_BYTES_LD];
	    uint8_t key_b_LDKEM[CRYPTO_BYTES_LD];

	    uint8_t kr_enc[2 * KYBER_SYMBYTES];
	    uint8_t kr_dec[2 * KYBER_SYMBYTES];

	    uint8_t encoder_flag;

	    ////////  Uncoded P_Kyber
	    printf("-------------------------------------------------------------------- \n");
	    printf("Test 2: Run the Uncoded P_Kyber KEM to exchange 8 AES Keys \n");
	    printf("*set encoder_flag = 0 for uncoded scheme \n");
	    //printf("*In params.h, set MMSE_Q_MODE = 'T' to enable MMSE quantization, or set MMSE_Q_MODE = 'F' to use the original Kyber quantization. \n");
	    printf("-------------------------------------------------------------------- \n");
	    encoder_flag = 0;
		...
		*DWT_CONTROL |= 1; // enable the counter
		...
		crypto_kem_keypair_P(pk_LDKEM, sk_LDKEM);

		t2 = *DWT_CYCCNT;
		*DWT_CONTROL |= 0; // close the counter
		numCycle1 = t2 - t1;
		printf("clock cycles for KeyGen: %u \n",
					numCycle1);                 // Display the difference

		*DWT_CONTROL |= 1; // enable the counter
		t1 = *DWT_CYCCNT;

		crypto_kem_enc_P(ct_LDKEM, key_a_LDKEM, pk_LDKEM, encoder_flag);

		t2 = *DWT_CYCCNT;
		*DWT_CONTROL |= 0; // close the counter
		numCycle1 = t2 - t1;
		printf("clock cycles for Enc: %u \n",
						numCycle1);

		*DWT_CONTROL |= 1; // enable the counter
		t1 = *DWT_CYCCNT;

		crypto_kem_dec_P(key_b_LDKEM, ct_LDKEM, sk_LDKEM, encoder_flag);

		t2 = *DWT_CYCCNT;
		*DWT_CONTROL |= 0; // close the counter
		numCycle1 = t2 - t1;
		printf("clock cycles for Dec: %u \n",
						numCycle1);


		printf("\n");
		printf("public key size: %u bytes \n",
				CRYPTO_PUBLICKEYBYTES_LD);
		printf("ciphertext size: %u bytes \n",
				KYBER_CIPHERTEXTBYTES_LD);


	    printf("\n");
	    printf("secrete message (8 AES keys): \n");
	    printUint8Array(key_a_LDKEM, KYBER_INDCPA_MSGBYTES_LD);
	    printf("\n");
	    printf("decrypted message (8 AES keys): \n");
	    printUint8Array(key_b_LDKEM, KYBER_INDCPA_MSGBYTES_LD);
	    printf("\n");
		   ...
		   ////////  E8 Coded P_Kyber
		    printf("-------------------------------------------------------------------- \n");
		    printf("Test 3: Run the E8 Coded P_Kyber KEM to exchange 8 AES Keys \n");
